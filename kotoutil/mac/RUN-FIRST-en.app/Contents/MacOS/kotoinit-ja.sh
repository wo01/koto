...
if [ -e ~/Library/Application\ Support/koto/koto.conf ]; then
    ...
    echo "gen=0                   #kotod立ち上げ時マニングを始めるか　gen=0 マイニングなし　gen1=マイニングする" >>~/Library/Application\ Support/koto/koto.conf
    echo "genproclimit=4          #使用スレッド数の指定" >>~/Library/Application\ Support/koto/koto.conf
    echo "exportdir = ~/Documents           #バックアップファイルの保存先" >>~/Library/Application\ Support/koto/koto.conf
    echo "mineaddress = k1KaxLaCbvng9qJYhBWzsoWBk8pdXoDFxfv   #ブロックを見つけた場合の振込先 ※自分のアドレスに変更して下さい!" >>~/Library/Application\ Support/koto/koto.conf
fi
