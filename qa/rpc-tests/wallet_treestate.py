...
class WalletTreeStateTest (BitcoinTestFramework):
    ...
    def run_test (self):
        ...
        recipients = []
        recipients.append({"address":self.nodes[2].z_getnewaddress(), "amount":Decimal('100.0') - Decimal('0.0001')})
        myopid = self.nodes[0].z_sendmany(mytaddr, recipients)
        wait_and_assert_operationid_status(self.nodes[0], myopid)
        ...
        recipients = []
        recipients.append({"address":self.nodes[2].z_getnewaddress(), "amount":Decimal('180')})
        recipients.append({"address":self.nodes[2].z_getnewaddress(), "amount":Decimal('111.9997') - Decimal('0.0001')})
